...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QMainWindow, QMessageBox, QApplication
...
class Window(QMainWindow):
    def __init__(self):
        ...
        # set icon
        self.setWindowIcon(QtGui.QIcon('screenshot.png'))
    ...
    def init_board(self):
        """create an empty list of list with None values, as soon as a piece is in it is final position,
...
class Piece:
    ...
    def move_down(self, rows, columns, board):
        ...
        test_position[4][0] += 1
        if check_position(test_position, rows, columns) and self.check_existing_pieces(test_position, board):
    ...
    def move_right(self, rows, columns, board):
        test_position = copy.deepcopy(self.piece)
        test_position[4][1] += 1
        if check_position(test_position, rows, columns):
